...
def get_quantiles(d,q,weights=None,num_bins=100):
	...
	for iz in range(nz):
		...
		cdf /= cdf[-1]
		# Interpolate the inverse CDF at the specified quantiles. Remove any empty bins
		# from the interpolation data to ensure that the x values are increasing.
		use = np.empty(len(cdf),dtype=bool)
		use[0] = True
		use[1:] = hist > 0
		inv_cdf = scipy.interpolate.InterpolatedUnivariateSpline(cdf[use],edges[use])
