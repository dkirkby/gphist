...
def main():
    ...
    # Load the input file.
    loaded = np.load('DESI_phi_test2.npz')
    ...
    DH_hist_DESI = loaded_DESI['DH_hist']
    DA_hist_DESI = loaded_DESI['DA_hist']
    de_hist_DESI = loaded_DESI['de_hist']
    f_hist_DESI = loaded_DESI['phi_hist']
    ...
    zvalues_DESI = loaded_DESI['zvalues']
    ...
    lna=-np.log(1 + zvalues[::-1])
    fixed_options = loaded_DESI['fixed_options']
    bin_range = loaded['bin_range']
    hyper_range = loaded_DESI['hyper_range']
    posterior_names = loaded_DESI['posterior_names']
    ...
    for iperm,j in zip((23,),(2,)):
        print posterior_names[perms[iperm]]
        ide = 0
        # Plot dark-energy evolution up to zmax.

        iend = 1+np.argmax(zvalues_bias > args.zmax)
        de_labels = (r'$\omega_{\Lambda}(z)/h_0^2$',r'$\omega_{\phi}(z)/h(z)^2$')
        assert len(de_labels) == len(de_hist_bias), 'Mismatch in number of dark-energy variables'
        ...
        de_limits = gphist.analysis.calculate_confidence_limits(
                    de_hist_bias[ide,iperm,:iend+1],[args.level],de_bin_range)
        interp_de_low = interp1d(zvalues_bias[:iend+1],de_limits[0,:iend+1],kind='cubic')
        interp_de_mid = interp1d(zvalues_bias[:iend+1],de_limits[1,:iend+1],kind='cubic')
        interp_de_high = interp1d(zvalues_bias[:iend+1],de_limits[2,:iend+1],kind='cubic')

        plt.subplot(gs[0])
        ax1 = plt.subplot(gs[0])
        ax1.set_yticks([-1.0,-0.5,0.0,0.5,1.0,1.5])
        plt.xscale('linear')
        #plt.grid(True)
        plt.xlim([0.,args.zmax])
        plt.ylim([-1.5,2])
        #plt.fill_between(zvalues_bias[:iend],de_limits[0],de_limits[-1],
        #            facecolor=plot_c[j],alpha=alpha_c[j])
        plt.fill_between(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True),interp_de_low(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True)),interp_de_high(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True)),color=plot_colors[j],alpha=alpha_c[j])
        #plt.plot(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True),interp_de_low(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True)),plot_c[j]+'-')
        plt.plot(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True),interp_de_mid(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True)),color=plot_colors[j],linestyle='-',linewidth=4.0)
        #plt.plot(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True),interp_de_high(np.linspace(0,zvalues_bias[iend],num=100,endpoint=True)),plot_c[j]+'-')
        #plt.plot(zvalues_bias[:iend],de_limits[0],plot_c[j]+'-')
        #plt.plot(zvalues_bias[:iend],de_limits[1],plot_c[j]+'-')
        #plt.plot(zvalues_bias[:iend],de_limits[2],plot_c[j]+'-')
        plt.plot(zvalues_bias[:iend],de0[ide,:iend],'k--',linewidth=4.0)
        plt.xlabel(r'$z$',fontsize=40)
        plt.ylabel(de_labels[ide],fontsize=40)



    for iperm,j in zip((31,63),(0,1)):
        print posterior_names[perms[iperm]]
        ide = 0
        # Plot dark-energy evolution up to zmax.

        iend = 1+np.argmax(zvalues_DESI > args.zmax)
        de_labels = (r'$\omega_{\Lambda}(z)/h_0^2$',r'$\omega_{\phi}(z)/h(z)^2$')
        ...
        de_limits = gphist.analysis.calculate_confidence_limits(
                    de_hist_DESI[ide,iperm,:iend+1],[args.level],de_bin_range)
        interp_de_low = interp1d(zvalues_DESI[:iend+1],de_limits[0,:iend+1],kind='cubic')
        interp_de_mid = interp1d(zvalues_DESI[:iend+1],de_limits[1,:iend+1],kind='cubic')
        interp_de_high = interp1d(zvalues_DESI[:iend+1],de_limits[2,:iend+1],kind='cubic')
        ...
        #            facecolor=plot_c[j],alpha=alpha_c[j])
        plt.fill_between(np.linspace(0,zvalues_DESI[iend],num=100,endpoint=True),interp_de_low(np.linspace(0,zvalues_DESI[iend],num=100,endpoint=True)),interp_de_high(np.linspace(0,zvalues_DESI[iend],num=100,endpoint=True)), color=plot_colors[j],alpha=alpha_c[j])
        ...
        plt.plot(np.linspace(0,zvalues_DESI[iend],num=100,endpoint=True),interp_de_mid(np.linspace(0,zvalues_DESI[iend],num=100,endpoint=True)),color=plot_colors[j],linestyle='-',linewidth=4.0)
        ...
        plt.plot(zvalues_DESI[:iend],de0[ide,:iend],'k--',linewidth=4.0)
        plt.xlabel(r'$z$',fontsize=40)
        plt.ylabel(de_labels[ide],fontsize=40)


    green_patch = mpatches.Patch(color=plot_colors[0],alpha=alpha_c[0],label = 'GP Result (present data)')
    red_patch = mpatches.Patch(color=plot_colors[1],alpha=alpha_c[1],label = 'With mock DESI data')
    blue_patch = mpatches.Patch(color =plot_colors[2],alpha=alpha_c[2],label = r'With mock w$_0$w$_a$ data')
    plt.subplot(gs[0])
    plt.legend(handles = [green_patch,red_patch,blue_patch],loc='lower left',fontsize=40,frameon=False)
