...
# DH
        ...
        plt.ylim([0.84,1.21])
        plt.fill_between(np.linspace(0,3,num=200,endpoint=True),interp_DH_low(np.linspace(0,3,num=200,endpoint=True)),interp_DH_high(np.linspace(0,3,num=200,endpoint=True)),
                         color=plot_colors[j],alpha=alpha_c[j])
...
#growth
        ...
        plt.fill_between(zvalues[:iend],filter_f_low(zvalues[:iend]),filter_f_high(zvalues[:iend]),
                        color=plot_colors[j],alpha=alpha_c[j])
        #plt.plot(zvalues[1:],np.ones(len(zvalues[1:])),'k:')
